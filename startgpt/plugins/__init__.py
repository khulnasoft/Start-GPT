...
from startgpt.models.base_open_ai_plugin import BaseOpenAIPlugin
...
def scan_plugins(config: Config, debug: bool = False) -> List[StartGPTPluginTemplate]:
    ...
    for plugin_path in [f.path for f in os.scandir(config.plugins_dir) if f.is_dir()]:
        ...
        for _, class_obj in inspect.getmembers(plugin):
            if (
                ...
                and StartGPTPluginTemplate in class_obj.__bases__
            ):
            ...
            for module in moduleList:
                ...
                for key in dir(zipped_module):
                    ...
                    if (
                        ...
                        and issubclass(a_module, StartGPTPluginTemplate)
                        and a_module.__name__ != "StartGPTPluginTemplate"
                    ):
                        ...
                        logger.debug(
                            f"Skipping {key}: {a_module.__name__} because it doesn't subclass StartGPTPluginTemplate."
                        )
