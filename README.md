...
1. Run the `startgpt` Python module in your terminal:
...
* `python scripts/main.py --ai-settings <filename>` to run Start-GPT with a different AI Settings file.
...
Use this to use TTS for Start-GPT

```
python -m startgpt --speak
```
...
By default Start-GPT is going to use LocalCache instead of redis or Pinecone.
...
This script located at scripts/data_ingestion.py, allows you to ingest files into memory and pre-seed it before running Start-GPT. 
...
You could for example download the documentation of an API, a Github repository, etc. and ingest it into memory before running Start-GPT. 

⚠️ If you use Redis as your memory, make sure to run Start-GPT with the WIPE_REDIS_ON_START set to False in your .env file.

⚠️For other memory backend, we currently forcefully wipe the memory when starting Start-GPT. To ingest data with those memory backend, you can call the data_ingestion.py script anytime during an Start-GPT run. 
...
```
python -m startgpt --speak --continuous
...
If you don't have access to the GPT4 api, this mode will allow you to use Start-GPT!

```
python -m startgpt --speak --gpt3only
```
...
By default, Start-GPT uses DALL-e for image generation. To use Stable Diffusion, a [HuggingFace API Token](https://huggingface.co/settings/tokens) is required.
...
```
flake8 startgpt/ tests/

# Or, if you want to run flake8 with the same configuration as the CI:
flake8 startgpt/ tests/ --select E303,W293,W291,W292,E305,E231,E302
```
