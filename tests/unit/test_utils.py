...
def test_get_bulletin_from_web_success(mock_get):
    ...
    mock_get.assert_called_with(
        "https://raw.githubusercontent.com/khulnasoft/Start-GPT/master/BULLETIN.md"
    )
...
def test_get_latest_bulletin_with_file():
    ...
    with patch("startgpt.utils.get_bulletin_from_web", return_value=""):
        bulletin, is_new = get_latest_bulletin()
        assert expected_content in bulletin
        assert is_new == False
...
def test_get_latest_bulletin_with_new_bulletin():
    ...
    with patch("startgpt.utils.get_bulletin_from_web", return_value=expected_content):
        bulletin, is_new = get_latest_bulletin()
