# Setting up Start-GPT

## 📋 Requirements

Choose an environment to run Start-GPT in (pick one):
...
!!! attention
    To use the OpenAI API with Start-GPT, we strongly recommend **setting up billing**
...
1. Clone the repository
        ...
        :::shell
        git clone -b stable https://github.com/khulnasoft/Start-GPT.git
...
1. Download `Source code (zip)` from the [latest stable release](https://github.com/khulnasoft/Start-GPT/releases/latest)
2. Extract the zip-file into a folder


### Configuration

1. Find the file named `.env.template` in the main `Start-GPT` folder. This file may
...
!!! info "Using a GPT Azure-instance"
    ...
    Example:
        ...
        azure_model_map:
            fast_llm_model_deployment_id: "<start-gpt-deployment>"
                ...
...
``` shell
docker-compose run --rm start-gpt --gpt3only --continuous
```

If you dare, you can also build and run it with "vanilla" docker commands:
``` shell
docker build -t start-gpt .
docker run -it --env-file=.env -v $PWD:/app start-gpt
docker run -it --env-file=.env -v $PWD:/app --rm start-gpt --gpt3only --continuous
```

[docker-compose file]: https://github.com/khulnasoft/Start-GPT/blob/stable/docker-compose.yml
...
``` shell
python -m venv venvStartGPT
source venvStartGPT/bin/activate
