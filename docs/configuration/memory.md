!!! warning
    ...
    feel free to pitch in: https://github.com/khulnasoft/Start-GPT/discussions/4280

## Setting Your Cache Type

By default, Start-GPT set up with Docker Compose will use Redis as its memory backend.
...
!!! important
    If you have set up Start-GPT using Docker Compose, then Redis is included, no further
    setup needed.
...
is supported which allows the Start-GPT process itself to start a Weaviate instance.
...
MEMORY_INDEX="Startgpt" # name of the index to create for the application
```
...
## 🧠 Memory pre-seeding
Memory pre-seeding allows you to ingest files into memory and pre-seed it before running Start-GPT.
...
GitHub documentation before running Start-GPT.

!!! attention
    If you use Redis for memory, make sure to run Start-GPT with `WIPE_REDIS_ON_START=False`

    For other memory backends, we currently forcefully wipe the memory when starting
    Start-GPT. To ingest data with those memory backends, you can call the
    `data_ingestion.py` script anytime during an Start-GPT run.

Memories will be available to the AI immediately as they are ingested, even if ingested
while Start-GPT is running.
