...
Configuration is controlled through the `Config` object. You can set configuration variables via the `.env` file. If you don't have a `.env` file, create a copy of `.env.template` in your `Start-GPT` folder and name it `.env`.
...
- `DISABLED_COMMAND_CATEGORIES`: Command categories to disable. Command categories are Python module names, e.g. startgpt.commands.execute_code. See the directory `startgpt/commands` in the source for all command modules. Default: None
...
- `HEADLESS_BROWSER`: Use a headless browser while Start-GPT uses a web browser. Setting to `False` will allow you to see Start-GPT operate the browser. Default: True
...
- `MEMORY_INDEX`: Value used in the Memory backend for scoping, naming, or indexing. Default: start-gpt
...
- `SHELL_ALLOWLIST`: List of shell commands that ARE allowed to be executed by Start-GPT. Only applies if `SHELL_COMMAND_CONTROL` is set to `allowlist`. Default: None
- `SHELL_COMMAND_CONTROL`: Whether to use `allowlist` or `denylist` to determine what shell commands can be executed (Default: denylist)
- `SHELL_DENYLIST`: List of shell commands that ARE NOT allowed to be executed by Start-GPT. Only applies if `SHELL_COMMAND_CONTROL` is set to `denylist`. Default: sudo,su
